          ...
          <el-table-column label="显示类型" min-width="12%">
            <template slot-scope="scope">
              <el-select v-model="scope.row.htmlType">
                ...
                <el-option label="日期时间控件" value="fulldatetime" />
            ...
            <template slot-scope="scope">
              <el-select v-model="scope.row.dictType" clearable filterable placeholder="请选择">
                <el-option v-for="dict in dictOptions" :key="dict.dictType" :label="dict.dictName" :value="dict.dictType">
                  <span class="left">{{ dict.dictName }}</span>
                  <span class="right">{{ dict.dictType }}</span>
                </el-option>
              </el-select>
            </template>
          </el-table-column>
    ...
    if (tableId) {
      ...
      getDictOptionselect().then(response => {
        ...
        this.dictOptions.push({
          dictType: "sys_data_user",
          dictName: "系统用户数据",
        });
        this.dictOptions.push({
          dictType: "sys_com_query",
          dictName: "通用查询",
        });
      });
      /** 查询通用查询下拉列表 */
      getComQueryOptionselect().then(res => {
        this.comQueryOptions = res.rows;
      });
      /** 查询菜单下拉列表 */
